...
*   **Robust Background Pipelines**: Managed by [Inngest](https://www.inngest.com) for event-driven, fault-tolerant execution steps, polling, and automated cleanups.
...
Below is the workflow showing the interaction between the Client, Next.js Server, Database, background workers (Inngest), and third-party APIs:

```mermaid
graph TD
    %% Styling
    classDef primary fill:#7C6AFA,stroke:#fff,stroke-width:2px,color:#fff;
    classDef secondary fill:#3ECFCF,stroke:#fff,stroke-width:2px,color:#05050A;
    classDef db fill:#05050A,stroke:#7C6AFA,stroke-width:1px,color:#fff;
    classDef external fill:#2b2b3a,stroke:#3ECFCF,stroke-width:1px,color:#fff;

    %% Elements
    A[Client UI] -->|1. Uploads Long Video| B(Next.js App Router API)
    B -->|2. Scans filename, WAF, limit| C[Arcjet Security]
    B -->|3. Writes local temp file| D[(Local Storage)]
    B -->|4. Registers Project| E[(Neon DB)]
    B -->|5. Triggers Background Job| F[Inngest Event Bus]

    F -->|6. Process Upload| G[processVideoUpload job]
    G -->|7. Uploads Video| H[AWS S3 Bucket]
    G -->|8. Generates presigned URL & saves| E
    G -->|9. Cleans up temp file| D

    F -->|10. Triggers Analysis| I[analyzeProjectVideo job]
    I -->|11. Transcribes Audio| J[Deepgram API]
    I -->|12. Scans transcript for injection| C
    I -->|13. Extracts Engagement clips| K[Gemini AI Model]
    I -->|14. Saves clips & word captions| E

    A -->|15. Requests Render| L(Next.js Clip Render API)
    L -->|16. Triggers Render Event| F
    F -->|17. Render Clip| M[renderShortVideoClip job]
    M -->|18. Invokes Headless Render| N[AWS Lambda / Remotion]
    N -->|19. Reads source & renders| H
    M -->|20. Polls progress & updates| E
    M -->|21. Saves export URL| E

    A -->|22. Connects Social Accounts| O(Zernio Integration API)
    O -->|23. Syncs and schedules posts| E
    O -->|24. Auth & Posting| P[Zernio Social API]

    %% Class Assign
    class A,B,L,O primary;
    class G,I,M secondary;
    class E,D db;
    class C,F,H,J,K,N,P external;
```

### Component Breakdown

1.  **Frontend (Next.js client-side UI)**: Uses [DM Sans](https://fonts.google.com/specimen/DM+Sans) and [Space Grotesk](https://fonts.google.com/specimen/Space+Grotesk), glassmorphism cards, and Framer Motion transitions. The player component integrates `@remotion/player` for live, real-time caption styling preview before rendering.
2.  **API Gateway (Next.js App Router)**: Standardized serverless routes for uploading media, querying project analysis statuses, generating captions, connecting social profiles, and scheduling posts.
...
4.  **Database Layer (Neon + Drizzle ORM)**: PostgreSQL schemas located in [lib/db/schema.ts](file:///Users/ishpaulsingh/Desktop/ai%20video%20generator%20and%20post%20scheduler/lib/db/schema.ts) that represent relations between user sync records, project statuses, short clip metadata, social platform credentials, and scheduled queues.
...
6.  **Background Processor (Inngest Dev/Production Server)**: An external runner that polls the Next.js API `/api/inngest` endpoint to run decoupled steps in [lib/inngest/functions.ts](file:///Users/ishpaulsingh/Desktop/ai%20video%20generator%20and%20post%20scheduler/lib/inngest/functions.ts) with concurrency limitations and automated state-polling retry logic.
...
3.  **Color System**: Reference colors from [theme.md](file:///Users/ishpaulsingh/Desktop/ai%20video%20generator%20and%20post%20scheduler/theme.md). Primary accent must be `--color-forge-accent` (`#7C6AFA`), secondary teal is `--color-forge-accent-2` (`#3ECFCF`). Avoid raw Tailwind base colors.
