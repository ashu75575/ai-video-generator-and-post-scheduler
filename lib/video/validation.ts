...
export async function validateVideo(filePath: string): Promise<VideoMetadata> {
  ...
  try {
    ...
    throw new Error(`Media validation failed: file is corrupted or not a valid video format. Details: ${err.message}`);
  }
  ...
  try {
    ...
    throw new Error("Media validation failed: unable to parse media properties.");
  }

  // 1. Check for video stream
  const videoStream = parsed.streams?.find((s: any) => s.codec_type === "video");
  if (!videoStream) {
    throw new Error("Media validation rejected: No video stream found in the uploaded file.");
  }

  // 2. Validate codec support
  const codec = videoStream.codec_name?.toLowerCase();
  if (!codec || !SUPPORTED_CODECS.has(codec)) {
    throw new Error(`Media validation rejected: Unsupported video codec "${codec || "unknown"}".`);
  }

  // 3. Validate format support
  const formatNameRaw = parsed.format?.format_name?.toLowerCase();
  if (!formatNameRaw) {
    throw new Error("Media validation rejected: Unable to identify video container format.");
  }

  const formats = formatNameRaw.split(",").map((f: string) => f.trim());
  const isFormatSupported = formats.some((f: string) => SUPPORTED_FORMATS.has(f));
  if (!isFormatSupported) {
    throw new Error(`Media validation rejected: Unsupported container formats [${formats.join(", ")}].`);
  }
  ...
  if (width <= 0 || height <= 0) {
    throw new Error(`Media validation rejected: Invalid video dimensions ${width}x${height}.`);
  }
